...
export const login = createAsyncThunk('auth/login',async(user,thunkAPI)=>{
    try {
        ...
        const message = (error.resposne && error.response.data && error.response.data.message) || error.message || error.toString()
        return thunkAPI.rejectWithValue(message)
    }
}) 
...
export const updateUser = createAsyncThunk('auth/updateUser',async(user,thunkAPI)=>{
    try {
        ...
        return await authServices.updateUser(user,token)
    } catch (error) {
        const message = (error.resposne && error.response.data && error.response.data.message) || error.message || error.toString()
        return thunkAPI.rejectWithValue(message)
    }
